...
snapshot_path = os.path.join(FLAGS.root_path, 'unet_supervisedonly_dp')
...
with open(FLAGS.root_path + '/preprocess/val.txt', 'r') as f:
    image_list = f.readlines()
image_list = [FLAGS.root_path +'/preprocess/' + item.replace('\n', '') + '_image.npy' for item in image_list]
print(image_list)

def test_calculate_metric(epoch_num):
    net = UNet3D(num_classes=num_classes, input_channels=1, act='relu', norm='in').cuda()
    save_mode_path = os.path.join(snapshot_path, 'checkpoint_' + str(epoch_num) + '.pth')
...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    ...
    for image_path in tqdm(image_list):
        ...
        prediction = test_single_case(net, image, num_classes=10)
...
def test_single_case(net, image, num_classes=10):
    score_map = np.zeros((num_classes, ) + image.shape).astype(np.float32)

    test_patch = np.expand_dims(image, axis=0).astype(np.float32)
