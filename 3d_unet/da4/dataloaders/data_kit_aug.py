...
class DataFolder(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.random.rand() > 0.2:
            ...
            image = np.load(os.path.join('/'.join(self.root_dir.split('/')[:-2]), 'aug', image_name + "_image.npy"))
            label = np.load(os.path.join('/'.join(self.root_dir.split('/')[:-2]), 'aug', image_name + "_label.npy"))
