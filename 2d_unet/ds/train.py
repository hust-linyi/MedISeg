...
# import imageio
import wandb
from utils.loss import dice_loss, compute_loss_list
...
def main():
    ...
    if 'res' in opt.model['name']:
        model = ResUNet34(net=opt.model['name'], seg_classes=2, colour_classes=3, pretrained=opt.model['pretrained'])    
    else:
        model = UNet(3, 2, 2)
...
def train(train_loader, model, optimizer, criterion, epoch, mix_loss='baseline', thr_epoch=30, thr_conf=0.05):
    ...
    for i, (input, gt, _) in enumerate(train_loader):
        ...
        # get multi-scale label
        gt = gt.float().cuda().view(gt.shape[0], 1, gt.shape[1], gt.shape[2])
        gt1 = F.interpolate(gt, scale_factor=0.5, mode='bilinear')
        gt2 = F.interpolate(gt, scale_factor=0.25, mode='bilinear')
        gt3 = F.interpolate(gt, scale_factor=0.125, mode='bilinear')
        gt, gt1, gt2, gt3 = gt.long().squeeze(1), gt1.long().squeeze(1), gt2.long().squeeze(1), gt3.long().squeeze(1)

        output, out1, out2, out3 = model(input)
        losses_ce = compute_loss_list(criterion, [output, out1, out2, out3], [gt, gt1, gt2, gt3])
        losses_dice = compute_loss_list(dice_loss, [output, out1, out2, out3], [gt, gt1, gt2, gt3])
        loss = 0.0
        for iloss in losses_ce:
            loss += iloss
        for iloss in losses_dice:
            loss += iloss
