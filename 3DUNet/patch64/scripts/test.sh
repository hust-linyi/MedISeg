cd /newdata/ianlin/CODE/seg_trick/3d_unet/patch64

python test.py --task patch64 --fold 0 --test-gpus 0 --test-test-epoch 0 --patch-size 64
python test.py --task patch64 --fold 1 --test-gpus 0 --test-test-epoch 0 --patch-size 64
python test.py --task patch64 --fold 2 --test-gpus 0 --test-test-epoch 0 --patch-size 64
python test.py --task patch64 --fold 3 --test-gpus 0 --test-test-epoch 0 --patch-size 64
python test.py --task patch64 --fold 4 --test-gpus 0 --test-test-epoch 0 --patch-size 64
