...
class GenericPreprocessor(object):
    ...
    def get_raw_training_data(self):
        ...
        patient_ids = [f for f in os.listdir(self.downloaded_data_dir) if isdir(join(self.downloaded_data_dir, f)) and f not in blacklist]
        print(f'Got total {len(patient_ids)} raw data')
        patient_ids.sort()
        for patient_id in tqdm.tqdm(patient_ids):
            img_1 = sitk.ReadImage(join(self.downloaded_data_dir, patient_id, 'imaging.nii.gz'))
            img_2 = sitk.ReadImage(join(self.downloaded_data_dir, patient_id, 'segmentation.nii.gz'))
